...
def hierarchical_clustering(
    ...
    # if it is a dataframe we take only the values, else we take all the matrix
    data = matrix.values if isinstance(matrix, pd.DataFrame) else matrix

    # Linkage matrix using scipy, we build the gerarchic tree
    ...
    if plot_dendrogram: # case in which we want to plote the dendrogram
        plt.figure()
        sch.dendrogram(
            linkage_matrix,
